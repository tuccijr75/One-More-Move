...
const MEMORY_STORE_KEY = "one-more-move-memoryStore-v1";
const memoryStore = {
    settings: {},
    difficulty: 'standard',
    bestScores: {},
...
function saveTuning(tuning) {
    memoryStore.settings[tuningKey()] = {
        ...tuning
    };
    saveMemoryStore();
}
...
function bestKey(seedMode) {
    return `one-more-move-best-v${SETTINGS_VERSION}`;
}
...
function planEnemyMoves(cfg) {
    const current = state.enemies.map(e => ({
        ...e
    }));
    const desired = [];
    ...
    current.forEach((enemy, idx) => {
        // IMPORTANT: use CURRENT snapshot for occupancy checks, not state.enemies
        ...
        for (const tile of candidates) {
            ...
            if (score > bestScore || (score === bestScore && dist < bestDist)) {
                best = {
                    ...
                    intentLock: difficulty === "standard" ? 1 : difficulty === "hard" ? 2 : difficulty === "hardcore" ? 3 : 2
                };
                bestScore = score;
                bestDist = dist;
            }
    ...
    if (state.hasExtraLife) {
        ...
        for (const k of reachable) {
            ...
            const minDist = Math.min(
                ...state.enemies.map(e => manhattan(e, {
                    x,
                    y
                }))
            );
...
async function resolveTurnAsync() {
    ...
    if (state.turns > state.best) {
        ...
        const key = bestKey();
        memoryStore.bestScores[key] = state.best;
        saveMemoryStore();
    }
...
function handleKeyDown(e) {
  ...
  "1", "2", "3", "r", "n", "m", "x", "y"
    ...
    if (
        now < state.rewardCooldownUntil && ["q", "e", "z", "c", "v", "b", "f", " "].includes(key)
    ) {
...
function initState({
    ...
    state = {
        ...
        enemies: [],
        turns: 0,
        best: Number(memoryStore.bestScores[bestKey()] || 0),
    ...
    settingsBackEl && settingsBackEl.addEventListener("click", closeSettings);
    settingsSaveEl && settingsSaveEl.addEventListener("click", persistSettingsFromUI);
    settingsResetEl && settingsResetEl.addEventListener("click", resetSettings);
